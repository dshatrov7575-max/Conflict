...
from domain.services.player_experiments import comparison, list_experiments, mutate_experiment
...
class ProductionPlayerG8Tests(PlayerExperimentsFixture,TestCase):
    ...
    def test_only_projection_complete_workspaces_and_draft_assessment_experiments_enable_g8_actions(self):
        source=SCRIPT.read_text(encoding="utf-8"); self.assertIn('app.dataset.projectionStatus !== "COMPLETE"',source); self.assertIn('item.status !== "DRAFT"',source)
    ...
    def test_ai_and_human_tabs_select_distinct_columns_and_never_overwrite_lanes(self):
        ...
        source=SCRIPT.read_text(encoding="utf-8"); self.assertIn('"ИИ_Значение":"Эксперт_Значение"',source); self.assertIn("Independent assessment lane",source)
        self.assertNotIn(focus_id,source)
    ...
    def test_retained_ticket_acknowledgement_commit_recovery_and_key_reuse_ui_are_exact(self):
        source=SCRIPT.read_text(encoding="utf-8"); self.assertIn("FOUNDATION_PLAYER_XLSX_IMPORT_TICKET_V1",source); self.assertIn("excluded_42_acknowledged:true",source); self.assertIn("Idempotency-Key",source); self.assertIn("new FormData",source); self.assertNotIn("file_base64",source)

    def test_unknown_blank_zero_confidence_review_flags_and_blocked42_render_distinctly(self):
        source=SCRIPT.read_text(encoding="utf-8"); html=TEMPLATE.read_text(encoding="utf-8"); self.assertIn('item.status === "UNKNOWN"',source); self.assertIn("18 METHOD_BLOCKED + 24 RECODING_REQUIRED",source); self.assertIn("Уверенность",html)
    ...
    def test_manual_value_successor_history_and_frozen_target_disabled_reasons_are_exact(self):
        ...
        self.assertFalse(ParameterValue.objects.filter(code=focus_id).exists())

    def test_general_comparison_shows_only_raw_series_without_mean_rank_consensus_or_fill(self):
        focus_id="G8-G9-FOCUS-R1"
        _,experiment,_=self.aggregate(); self.create_value(experiment,value=0); result=comparison(user=self.user,workspace_id=self.workspace.pk); self.assertIsNone(result["aggregation"]); self.assertEqual(result["values"][0]["value"],0)
        self.assertFalse(any(row.get("focus",{}).get("id")==focus_id for row in result["values"]))
...
class ProductionPlayerG8ChromiumTests(PlayerExperimentsFixture,StaticLiveServerTestCase):
    ...
    def run_browser(self,scenario):
        ...
        transfers=[row["legacy_id"] for row in __import__("domain.services.xlsx_import_profiles",fromlist=["load_profile"]).load_profile()["records"] if row["migration_status"]=="TRANSFER_WITH_REVIEW"]
        with tempfile.TemporaryDirectory(prefix="g8-browser-xlsx-") as directory:
            ...
            boundaries=directory/"unknown-zero.xlsx"; boundaries.write_bytes(profile_workbook(unknown={transfers[0]},zero={transfers[1]},stored=True))
