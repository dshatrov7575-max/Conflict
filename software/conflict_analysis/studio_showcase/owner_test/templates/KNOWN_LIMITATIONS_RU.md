...
- Данные существуют только в текущей браузерной session-only сессии; публикации
  и изменения Foundation ORM/production database нет.
...
- Чат отключён; Help локален. Первичная установка зависимостей из PyPI требует
  сети (если нет локального pip-кэша), дальнейший локальный запуск — нет.
- Пакет не заявляет научную валидацию, production readiness, публикацию данных,
  прогнозирование или поддержку принятия решений.
