...
from verify_owner_alpha_package import (
    ...
    rootfs_inventory, safe_rootfs_member, sha256_file, source_identity, delivery_identity, verify_zip,
)
...
ZIP_NAME = "conflict-analysis-partner-demo-win-x64-mvp7.zip"
...
def download(pin, directory):
    ...
    cache = os.environ.get("MVP7_INPUT_CACHE")
    if not target.exists() and cache and (Path(cache)/pin["filename"]).is_file():
        shutil.copyfile(Path(cache)/pin["filename"],target)
    ...
    with tarfile.open(source) as incoming, tarfile.open(target, "w", format=tarfile.GNU_FORMAT) as out:
        ...
        for member in incoming:
            ...
            if not name or name in ignored or member.isdev() or member.isfifo():
                continue
            require(safe_rootfs_member(name) and name not in members,
...
def package_members(package, rootfs, source, wheel, runtime_report, inventory, delivery):
    ...
    manifest = {"schema": SCHEMA, "package_version": PACKAGE_VERSION, "source": source,
                "delivery": delivery, "acceptance": {"acceptance_run":35216652773,"WINDOWS11_WSL2_E2E":"BLOCKED_NO_RUNNER","CLEAN_PC_SMOKE":"NOT_EXECUTED","PARTNER_RELEASE_READY":False},
...
def build(repository, output):
    ...
    with tempfile.TemporaryDirectory(prefix="g10-build-") as temporary:
        ...
        members = package_members(package, output / ROOTFS_NAME, source, wheel, reports[0], inventory, delivery_identity(repository))
...
def regressions(repository, output):
    # G10's historical 350/322/68/10 totals do not describe R1.
    # The installer workflow runs the full accepted R1 gate in a separate
    # exact-C checkout. Never silently substitute old G10 acceptance here.
    raise RuntimeError("MVP7_R1_REGRESSIONS_REQUIRE_EXACT_C_WORKFLOW_JOB")
