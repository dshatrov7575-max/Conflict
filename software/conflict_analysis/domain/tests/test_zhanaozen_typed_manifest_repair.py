...
class ZhanaozenRepairIntegrityTests(TestCase):
    ...
    def test_post_probe_exact_concurrent_project_is_no_write_replay(self):
        project = _accepted_legacy_seed()
        defaults = self._project_defaults(project)
        updated_at = project.updated_at
        real_filter = Project.objects.filter
        pk_probes = 0

        def hide_post_identity_probe(*args, **kwargs):
            nonlocal pk_probes
            queryset = real_filter(*args, **kwargs)
            if kwargs == {"pk": project.pk}:
                pk_probes += 1
                if pk_probes == 2:
                    queryset.first = lambda: None
            return queryset

        with patch.object(Project.objects, "filter", side_effect=hide_post_identity_probe), patch.object(
            Project.objects,
            "update_or_create",
            side_effect=AssertionError("concurrent winner must not use update_or_create"),
        ), patch.object(
            Project,
            "save",
            side_effect=AssertionError("concurrent winner must not be saved"),
        ):
            replay = _upsert(
                Project,
        ...
        self.assertEqual(pk_probes, 2)
        self.assertEqual(replay.pk, project.pk)
        self.assertEqual(replay.updated_at, updated_at)
        self.assertEqual(Project.objects.get(pk=project.pk).updated_at, updated_at)

    def test_post_probe_different_concurrent_project_is_conflict(self):
        project = _accepted_legacy_seed()
        defaults = self._project_defaults(project)
        updated_at = project.updated_at
        Project.objects.filter(pk=project.pk).update(name="Concurrent drift")
        real_filter = Project.objects.filter
        pk_probes = 0
        ...
        with patch.object(Project.objects, "filter", side_effect=hide_post_identity_probe), patch.object(
            Project.objects,
            "update_or_create",
            side_effect=AssertionError("concurrent winner must not be reconciled"),
        ), patch.object(
            Project,
            "save",
            side_effect=AssertionError("concurrent winner must not be saved"),
        ...
        self.assertEqual(pk_probes, 2)
        persisted = Project.objects.get(pk=project.pk)
        self.assertEqual(persisted.name, "Concurrent drift")
        self.assertEqual(persisted.updated_at, updated_at)
    ...
    def test_postgresql_concurrent_bootstrap(self):
        ...
        def install():
            ...
            try:
                ...
                project = seed_zhanaozen_demo()
                results.append(("installed", str(project.pk), project.updated_at))
            except SeedConflictError as exc:
                results.append(("conflict", str(exc), None))
            except BaseException as exc:
                results.append(("unexpected", repr(exc), None))
            finally:
        ...
        self.assertNotIn("unexpected", [status for status, _, _ in results], results)
        successful = [result for result in results if result[0] == "installed"]
        conflicts = [result for result in results if result[0] == "conflict"]
        self.assertGreaterEqual(len(successful), 1, results)
        self.assertTrue(all(result[1] == typed.PROJECT_ID for result in successful), results)
        self.assertTrue(
            len(successful) == 2 or (len(successful) == 1 and len(conflicts) == 1),
            results,
        )
        project = Project.objects.get(pk=typed.PROJECT_ID)
        self.assertTrue(
            all(result[2] == project.updated_at for result in successful),
            results,
        )
        if len(successful) == 2:
            self.assertEqual(len({result[2] for result in successful}), 1, results)
        _verify_typed_installation(project)
